...
import com.j2swift.util.HeaderMap;
import com.j2swift.util.PackagePrefixes;
import com.j2swift.util.Version;

/**
 * The set of tool properties, initialized by the command-line arguments. This
 * class was extracted from the main class, to make it easier for other classes
 * to access options.
 *
 * @author Tom Ball
 */
public class Options {
	...
	private boolean jreTranslation = false;

	private PackagePrefixes packagePrefixes = new PackagePrefixes();

	private static final Set<String> VALID_JAVA_VERSIONS = ImmutableSet.of(
			"1.8", "1.7", "1.6", "1.5");

	// TODO(kirbs): Uncomment following lines and lines in OptionsTest when we
	// enable automatic
	// version detection. Currently this is breaking pulse builds using 64 bit
	// Java 8, and upgrading
	// to Eclipse 4.5 is gated by bytecode errors in compiling junit. I won't
	// have time to do a
	// more in depth root cause analysis on this.
	private String sourceVersion = "1.7";
	// // The default source version number if not passed with -source is
	// determined from the system
	// // properties of the running java version after parsing the argument
	// list.
	...
	// Null if not set (means we use the default). Can be empty also (means we
	// use no mapping files).
	...
	private static String bootclasspath = System
			.getProperty("sun.boot.class.path");
	private static final String BATCH_PROCESSING_MAX_FLAG = "--batch-translate-max=";

	static {
		// Load string resources.
		URL propertiesUrl = Resources.getResource(J2Swift.class,
				"JrePrefixesFile.properties");
	...
	public static enum MemoryManagementOption {
		REFERENCE_COUNTING, ARC
	}

	/**
	 * Types of output file generation. Output files are generated in the
	 * specified output directory in an optional sub-directory.
	 */
	public static enum OutputStyleOption {
		...
		/**
		 * Use the relative directory of the input file, even (especially) if it
		 * is a jar.
		 */
		SOURCE_COMBINED,
	...
	public static final OutputStyleOption DEFAULT_OUTPUT_STYLE_OPTION = OutputStyleOption.PACKAGE;
	 ...
	 * detected, the appropriate status method is invoked and the app
	 * terminates.
	 * 
	 * @throws IOException
	 */
	...
	private void addJreMappings() throws IOException {
		// InputStream stream =
		// J2ObjC.class.getResourceAsStream(JRE_MAPPINGS_FILE);
	...
	private void addMappingsProperties(Properties mappings) {
		...
		while (keyIterator.hasMoreElements()) {
			...
			if (key.indexOf('(') > 0) {
				// All method mappings have parentheses characters, classes
				// don't.
	...
	/**
	 * Adds a class, method or package-prefix property to its map, reporting an
	 * error if that mapping was previously specified.
	 */
	private static void addMapping(Map<String, String> map, String key,
			String value, String kind) {
		String oldValue = map.put(key, value);
		if (oldValue != null && !oldValue.equals(value)) {
			ErrorUtil.error(kind + " redefined; was \"" + oldValue + ", now "
					+ value);
		}
	}
	...
	private static List<String> getPathArgument(String argument) {
		...
		for (String entry : Splitter.on(File.pathSeparatorChar).split(argument)) {
			if (new File(entry).exists()) { // JDT fails with bad path entries.
				...
				String expanded = System.getProperty("user.home")
						+ entry.substring(1);
	...
	/**
	 * If true, put output files in sub-directories defined by package
	 * declaration (like javac does).
	 */
	...
	/**
	 * If true, put output files in the same directories from which the input
	 * files were read.
	 */
	...
	public static boolean isJava8Translator() {
		return instance.forceIncompleteJava8Support
				&& instance.sourceVersion.equals("1.8");
	}
	
	public static boolean isJreTranslation() {
		return instance.jreTranslation;
	}
